...
class MatrixGArray(GArray):
    ...
    def __init__(self, data, p="int"):
        ...
        if p == "int" and data.dtype != int:
            raise ValueError(
                "data.dtype must be int when integer parameterization is used."
            )

        if "mat" not in self.parameterizations and "hmat" not in self.parameterizations:
            raise AssertionError(
                'Subclasses of MatrixGArray should always have a "mat" and/or "hmat" parameterization'
            )
    ...
    def left_action_hvec(self, other):
        ...
        out = (
            np.einsum("...ij,...j->...i", self_mat, other.data)
            + self_hmat[..., :-1, -1]
        )
        return other.factory(data=out, p=other.p)
